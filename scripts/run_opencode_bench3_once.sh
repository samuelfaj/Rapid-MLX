...
if [ "$#" -lt 2 ]; then
  echo "usage: $0 PROFILE MODEL FLAGS..." >&2
  exit 2
fi
...
ROOT="/Users/samuelfajreldines/Rapid-MLX-ddtree"
BASE="/tmp/rapid-mlx-bench3"
...
PROMPT="create a REST aopencode using express and bun and typescript and sequelize-typescript. It must be vertical sliced. You should create models, seeders and migrations. You must create unit tests for each service."
...
(
  cd "$ROOT" && uv run rapid-mlx serve "$MODEL" \
    --served-model-name local --port 8010 --default-temperature 0 \
